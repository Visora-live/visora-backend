...
async def lifespan(app: FastAPI):
    # Pre-warm weapon worker (paused) so models are loaded before user clicks button
    prewarm = os.getenv("DETECTION_PREWARM_CAMERAS", "")
    for cam_str in prewarm.split(","):
        ...
        if cam_str.isdigit():
            ...
            detection_manager._spawn(
                detection_manager._WEAPON_PROCESSES,
                detection_manager._WEAPON_PYTHON,
    ...
    for cam_id in list(detection_manager._WEAPON_PROCESSES.keys()):
        detection_manager.kill(cam_id)
    for cam_id in list(detection_manager._FACE_PROCESSES.keys()):
        detection_manager.kill_face(cam_id)
...
app.include_router(algorithm.router, prefix=settings.API_PREFIX, **_protected)
